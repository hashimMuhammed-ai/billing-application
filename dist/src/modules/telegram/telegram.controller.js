...
let TelegramController = class TelegramController {
    ...
    constructor(telegramHandlerService) {
        this.telegramHandlerService = telegramHandlerService;
    }
    async handleWebhook(update, secretHeader) {
        const expectedSecret = process.env.TELEGRAM_WEBHOOK_SECRET;
        if (expectedSecret && secretHeader !== expectedSecret) {
            throw new common_1.UnauthorizedException('Invalid Telegram webhook secret token');
        }
...
exports.TelegramController = TelegramController = __decorate([
    ...
    __metadata("design:paramtypes", [telegram_handler_service_1.TelegramHandlerService])
], TelegramController);
